...
6. Self-driving car's decision making
   ...
   3. Behavior selection
      ...
      - A self-driving car has to deal with various road and urban traffic situations, and some of them simultaneously (e.g. yield and merge intersections). In the literature of the behavior selection problem can be divided according to the different traffic scenarios in order to solve them in parts.
        - The principal methods used for different driving scenarios were combination of heuristics, decision trees, and Finite State Machines (FSM).
        - State Machines based methods have been improved and fused with other methods to cope with a large variety of real urban traffic  scenario.
        - Ontologies can also be sued as a tool to model traffic scenarios.
          - Several approaches for behavior selection consider uncertainty in the intentions and trajectories of moving objects.
          - For that, they use probabilistic methods, such as Markov Decision Processes (MDPs) and Partially-Observable Markov Decision process (POMDP).
      - FSM-based techniques
      - Ontology-based techniques
      - Markov decision process based techniques
      
   4. Motion planning

      - The **Motion Planner** subsystem is responsible for computing a Trajectory, $T$, from the current self-driving car's State to the current Goal. 
      - The trajectory must follow the Path, $P_j$, defined by the Behavior Selector subsystem as close as possible, while satisfying car's kinematic and dynamic constraints, and proving safety and comfort to the passengers.
        - Graph search based techniques
          - State lattice
          - Elastic Band (EB)
          - A-star
